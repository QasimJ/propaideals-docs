...
**Fix:** Upgrade your plan, or use a different endpoint. Note the Free plan carries a reduced scope set — `properties:read`, `market-data:read`, `areas:read`, and `epc:read` only. See [Authentication › Scopes](./authentication#scopes).
...
```json
{
  "error": {
    ...
    "message": "AI chat quota exceeded (100/100). Upgrade your plan or wait until the next billing period.",
    "used": 100,
    "limit": 100
  }
}
...
AI chat is included from the Growth plan up: Growth 25/month, Advanced 100/month, Professional 1,000/month, Business 4,000/month. The Free and Starter plans do not include AI chat — calls to `/api/v1/ultimate-ai/*` on those plans fail on scope, not quota.

**Fix:** Upgrade for a higher AI chat quota, or wait until the first of next month when usage counters reset. Non-AI endpoints continue to work normally until your main monthly quota is also exhausted.
...
**Fix:** Check the [supported endpoint list](./endpoints/properties). If you need the requested endpoint exposed publicly, email `info@propaideals.co.uk`.
