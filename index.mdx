...
1. **[Quickstart](./quickstart)** — Make your first request in under 3 minutes (free — 200 credits/month, no card)
2. **[Authentication](./authentication)** — Create and use API keys
3. **[Rate limits](./rate-limits)** — Per-second and monthly quotas, plan comparison
4. **[Errors](./errors)** — Error codes and how to handle them
5. **[Response format](./response-format)** — Envelope, pagination, headers
6. **[Connect to Claude (MCP)](./connect-to-claude)** — Query the data from Claude with no code at all
