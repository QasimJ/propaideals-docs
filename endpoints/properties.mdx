...
| `property_type` | string\|array | The canonical vocabulary: `flat`, `terraced`, `semi-detached`, `detached`, `bungalow`, `house`, `studio`, `maisonette`, `land`, `commercial`, `parking`, `caravan`, `other`. Portal and Land Registry spellings (`Purpose Built Flat`, `semi_detached`, `Mid Terrace House`) are normalised server-side, so anything a [comparable](./comparables) publishes can be sent straight back here. `house` widens to every house type; `bungalow` also matches bungalows stored under their parent type. |
