...
- **`id` is a stable machine identifier and may predate a display rename.** The £99 Advanced plan was previously named Starter and keeps its original id, so existing integrations and subscriptions are unaffected. Match on `id` in code; show `name` to users.
- **The Free plan does not appear here.** This endpoint lists plans you can purchase; the Free plan (200 credits/mo, no card) is granted automatically — just [create a key](#create-an-api-key).
- **The new £29 Starter and £59 Growth tiers appear in this list as they go live.** Until then, the paid ladder shown at checkout is Advanced (£99), Professional (£299), and Business (£999). See [Rate limits › Plan comparison](../rate-limits#plan-comparison) for the full ladder.
- Enterprise plans are provisioned manually (contact `info@propaideals.co.uk`).
