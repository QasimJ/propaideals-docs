...
All limits are per **account** (not per IP, not per key). Your subscription buys one shared pool: every API key you create draws from the **same** per-second rate limit, monthly request quota, and AI chat quota. Creating more keys does **not** increase your limits. [MCP tool calls](./connect-to-claude) (the Claude connector) draw from the same monthly pool as your REST keys.
